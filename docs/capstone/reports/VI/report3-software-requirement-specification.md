...
- **Chấm Điểm AI** cho Viết và Nói sử dụng LLM (Groq Llama 3.3 70B) và Speech-to-Text (Groq Whisper Large V3 Turbo)
...
- **Công cụ Quản trị viên** cho quản lý người dùng, quản lý nội dung và phân tích
...
Biểu đồ ngữ cảnh bên dưới minh họa các thực thể bên ngoài và giao diện hệ thống cho Hệ Thống Luyện Thi VSTEP Thích Ứng. Ranh giới hệ thống bao gồm các client Web/Di động, Máy chủ API Chính, Worker Chấm Điểm, PostgreSQL và Redis. Các thực thể bên ngoài bao gồm ba vai trò người dùng (Học viên, Giảng viên, Quản trị viên) và các API nhà cung cấp AI (Groq LLM và Groq Whisper).
...
- **Shared-DB**: Cả Ứng dụng Chính và Worker Chấm Điểm đều kết nối tới cùng cơ sở dữ liệu PostgreSQL. Worker ghi kết quả chấm điểm trực tiếp — không cần callback queue hay outbox pattern.
- **Redis Queue**: Danh sách Redis với LPUSH/BRPOP thay thế RabbitMQ. Đơn giản hơn, ít thành phần hơn.
...
|-----------|----------|-------------|
| Client ↔ Main App | REST (HTTPS) + SSE | Mọi tương tác người dùng qua REST API; cập nhật chấm điểm thời gian thực qua SSE |
| Main App ↔ PostgreSQL | TCP (PostgreSQL wire protocol) | Drizzle ORM cho truy cập dữ liệu |
| Main App ↔ Redis | RESP3 (Bun built-in) | Đẩy hàng đợi, cache, giới hạn tốc độ, khóa claim đánh giá |
| Grading Worker ↔ Redis | RESP3 | BRPOP để lấy công việc từ hàng đợi |
| Grading Worker ↔ PostgreSQL | TCP (psycopg/asyncpg) | Ghi trực tiếp kết quả chấm điểm |
| Grading Worker ↔ Nhà cung cấp AI | HTTPS | LLM (Groq Llama 3.3) cho chấm điểm, Whisper (Groq) cho STT |
...
| ID | Tính năng | Mô tả |
|----|---------|-------------|
| FE-12 | Quản Lý Nội Dung | CRUD ngân hàng câu hỏi, nhập/xuất (Excel, JSON) |
| FE-13 | Quản Lý Người Dùng | Tạo tài khoản hàng loạt, khóa/mở khóa, đặt lại mật khẩu, phân quyền vai trò |
| FE-14 | Bảng Phân Tích | Người dùng hoạt động, tỷ lệ hoàn thành, điểm trung bình, lọc theo thời gian |
| FE-15 | Hệ Thống Thông Báo | Thông báo đẩy (di động), email, thông báo trong ứng dụng |
| FE-16 | Tính Năng Quản Trị Nâng Cao | Lịch sử hoạt động, phân phối bài chấm tự động |
...
| **Giảng viên** | Giảng viên tiếng Anh tại các trường đại học hoặc trung tâm ngôn ngữ. Chịu trách nhiệm đánh giá các bài nộp Viết/Nói đã được AI chấm điểm và cung cấp phản hồi. | Trung bình. Thoải mái với ứng dụng web và phân tích dữ liệu cơ bản. |
| **Quản trị viên** | Quản trị hệ thống quản lý người dùng, ngân hàng câu hỏi và cấu hình nền tảng. | Nâng cao. Quen thuộc với hệ thống quản lý nội dung và quản trị dữ liệu. |
...
| C-06 | Phụ Thuộc Nhà Cung Cấp LLM | Chấm điểm AI phụ thuộc vào nhà cung cấp LLM/STT bên ngoài (Groq, OpenAI) với giới hạn tốc độ |
...
- A-01: Học viên có kết nối internet ổn định (tối thiểu 1 Mbps cho phát trực tuyến âm thanh)
...
- A-04: Nhà cung cấp AI (Groq, OpenAI) duy trì khả dụng dịch vụ và tương thích API
- A-05: PostgreSQL và Redis sẵn sàng và hoạt động trong môi trường triển khai

**Phụ thuộc:**

- D-01: Groq API (Llama 3.3 70B) cho chấm điểm Viết/Nói dựa trên LLM
- D-02: Groq API (Whisper Large V3 Turbo) cho phiên âm Speech-to-Text
- D-03: PostgreSQL 17 cho lưu trữ dữ liệu
- D-04: Redis 7.2+ cho hàng đợi tác vụ, bộ nhớ đệm và khóa phân tán
- D-05: Docker Compose cho điều phối môi trường phát triển cục bộ
...
| 2 | **Giảng viên** | Người dùng (role = `instructor`) tạo bài thi và câu hỏi, đánh giá các bài nộp Viết/Nói đã được AI chấm, quản lý lớp học, giám sát tiến độ học viên và cung cấp phản hồi. Kế thừa tất cả quyền của Học viên. |
| 3 | **Quản trị viên** | Người dùng (role = `admin`) quản lý người dùng, ngân hàng câu hỏi, cấu hình hệ thống và xem bảng phân tích. Kế thừa tất cả quyền của Giảng viên. |
| 4 | **Grading Worker** | Tác nhân hệ thống tự động (Python + FastAPI) tiêu thụ tác vụ chấm điểm từ Redis, thực hiện chấm điểm AI qua API LLM/STT bên ngoài và ghi kết quả vào cơ sở dữ liệu. |
| 5 | **Nhà cung cấp AI (Groq)** | Dịch vụ bên ngoài cung cấp suy luận LLM (Llama 3.3 70B) cho chấm điểm Viết/Nói và phiên âm STT (Whisper Large V3 Turbo) cho bài nộp Nói. |
...
| UC-12 | Luyện Viết | Học viên | Nộp bài viết. Chấm điểm AI bất đồng bộ qua Redis queue + LLM. Bao gồm UC-14. |
| UC-13 | Luyện Nói | Học viên | Nộp bản ghi âm. STT + chấm điểm AI bất đồng bộ qua Redis queue. Bao gồm UC-14. |
| UC-14 | Xem Kết Quả Bài Nộp | Học viên | Xem điểm, band, chi tiết từng tiêu chí cho bài nộp đã hoàn thành. |
| UC-15 | Xem Phản Hồi AI | Học viên | Xem chi tiết phản hồi AI, lỗi ngữ pháp (viết), ghi chú phát âm (nói). |
| UC-20 | Xem Danh Sách Bài Thi | Học viên | Duyệt các bài thi thử lọc theo cấp độ (B1/B2/C1). |
| UC-21 | Bắt Đầu Phiên Thi | Học viên | Bắt đầu phiên thi 4 kỹ năng có giới hạn thời gian. Tạo `exam_session` với trạng thái `in_progress`. |
| UC-22 | Trả Lời Câu Hỏi Thi | Học viên | Trả lời câu hỏi trong kỳ thi. Tự động lưu mỗi 30 giây. |
| UC-23 | Nộp Bài Thi | Học viên | Hoàn tất bài thi. L/R chấm tự động inline; W/S gửi tới Redis để chấm điểm AI. |
| UC-24 | Xem Kết Quả Thi | Học viên | Xem điểm từng kỹ năng, band và điểm tổng khi tất cả kỹ năng đã được chấm. |
...
| UC-51 | Nhận Bài Chấm | Giảng viên | Khóa bài nộp để đánh giá độc quyền (15 phút TTL). |
...
| UC-60 | Tạo Bài Thi | Giảng viên | Định nghĩa bài thi với cấp độ và blueprint (chọn câu hỏi theo kỹ năng). |
...
| UC-71 | Xem Bảng Điều Khiển Lớp | Giảng viên | Thống kê tổng hợp lớp: trung bình từng kỹ năng, học viên có nguy cơ (trung bình < 5.0). |
| UC-72 | Xem Tiến Độ Thành Viên | Giảng viên | Tiến độ học viên cá nhân: điểm từng kỹ năng, xu hướng, trạng thái mục tiêu. |
...
| UC-80 | Danh Sách Người Dùng | Quản trị viên | Danh sách người dùng phân trang với bộ lọc (vai trò, tìm kiếm email/tên). |
| UC-81 | Đổi Vai Trò | Quản trị viên | Thay đổi vai trò người dùng giữa learner/instructor/admin. |
| UC-82 | Khóa/Mở Khóa Tài Khoản | Quản trị viên | Vô hiệu hóa/kích hoạt quyền truy cập tài khoản người dùng. |
| UC-83 | Đặt Lại Mật Khẩu | Quản trị viên | Đặt lại mật khẩu do quản trị viên khởi tạo cho người dùng. |
| UC-84 | Tạo Người Dùng Hàng Loạt | Quản trị viên | Tạo nhiều tài khoản từ tải lên CSV/Excel. |
| UC-85 | Quản Lý Ngân Hàng Câu Hỏi | Quản trị viên | CRUD đầy đủ trên câu hỏi. Xóa mềm qua `is_active = false`. |
| UC-86 | Nhập/Xuất Câu Hỏi | Quản trị viên | Nhập hàng loạt từ JSON/Excel; xuất để sao lưu. |
| UC-87 | Quản Lý Điểm Kiến Thức | Quản trị viên | CRUD trên phân loại điểm kiến thức (grammar, vocabulary, strategy). |
| UC-88 | Cập Nhật Bài Thi | Quản trị viên | Chỉnh sửa blueprint và cấp độ bài thi. |
| UC-90 | Xem Bảng Quản Trị | Quản trị viên | Phân tích toàn hệ thống: tổng người dùng, phân bố bài nộp, kích thước hàng đợi. |
| UC-91 | Xem Phân Tích Hệ Thống | Quản trị viên | Người dùng hoạt động hàng ngày, thời gian chấm điểm trung bình, tỷ lệ hoàn thành. |
| UC-92 | Kích Hoạt Chấm Tự Động | Quản trị viên | Kích hoạt thủ công chấm tự động cho các bài nộp Nghe/Đọc đang chờ. |
...
| Quản Trị — Khóa/Mở Khóa | | | X |
| Quản Trị — Bảng Phân Tích | | | X |
| Quản Trị — Kích Hoạt Chấm Tự Động | | | X |
...
|---|---------|----------------|-------------|
| 1 | Chấm Điểm AI | Grading Worker (BRPOP) | Worker bất đồng bộ lấy từ danh sách Redis `grading:tasks`. Định tuyến tới pipeline chấm Viết hoặc Nói. Ghi kết quả vào PostgreSQL. |
...
| 3 | Chấm Điểm AI | Hàng Đợi Thất Bại | Tác vụ thất bại sau tối đa lần thử → `LPUSH grading:dlq` để kiểm tra thủ công. |
...
| 7 | Thi Thử | Tự Động Lưu | Client gửi snapshot câu trả lời mỗi 30 giây → upsert `exam_answers`. Từ chối nếu phiên đã nộp. |
| 8 | Thi Thử | Xử Lý Nộp Bài | Khi nộp bài thi: chấm tự động L/R inline → tạo bài nộp W/S → gửi tới Redis → cập nhật điểm phiên. |
...
| 10 | `knowledge_points` | Phân loại điểm kiến thức (grammar, vocabulary, strategy) cho học tập thích ứng. |
...
- **Mô tả**: Khởi tạo bài đánh giá xếp lớp bao gồm tất cả bốn kỹ năng để xác định trình độ năng lực ban đầu.
- **Đầu vào**: Yêu cầu học viên đã xác thực
- **Xử lý**: Chọn bộ câu hỏi cân bằng giữa các kỹ năng và mức độ khó → tạo phiên xếp lớp
...
- **Đầu vào**: ID phiên, câu trả lời cho tất cả bốn kỹ năng
- **Xử lý**: Chấm tự động Nghe/Đọc qua đáp án → tạo bài nộp Viết/Nói cho chấm điểm AI → tính band ban đầu mỗi kỹ năng → khởi tạo bản ghi `user_progress` → khởi tạo Biểu đồ Spider
- **Đầu ra**: Band mỗi kỹ năng (A1–C1), mức hỗ trợ ban đầu được đề xuất, lộ trình học tập gợi ý
...
- **Xử lý**:
  ...
  4. LPUSH tác vụ chấm điểm vào danh sách Redis `grading:tasks`
  5. Trả về ID bài nộp với URL SSE để theo dõi trạng thái
- **Đầu ra**: `{ submissionId, status: "pending", sseUrl: "/api/sse/submissions/{id}" }`
- **Pipeline Chấm Điểm AI** (bất đồng bộ trong worker):
  ...
  - Mô hình ngữ pháp chạy song song để phát hiện lỗi chi tiết
...
- **Mô tả**: Endpoint polling cho các client không thể sử dụng SSE.
- **Đầu vào**: ID bài nộp
...
- **Xử lý**: Luồng tương tự như Viết (xác thực → tạo bài nộp → đẩy vào Redis → trả URL SSE). Xem [Phần 3.6.4 — Biểu Đồ Pipeline Chấm Điểm AI](#364-biểu-đồ-pipeline-chấm-điểm-ai-viếtnói).
- **Pipeline Chấm Điểm AI** (bất đồng bộ trong worker):
  - Whisper phiên âm âm thanh thành văn bản
...
- **Đầu vào**: ID phiên
- **Xử lý**:
  1. Chấm tự động câu trả lời Nghe: so sánh với đáp án → `listening_score = (correct/35) × 10` (làm tròn tới 0.5)
  2. Chấm tự động câu trả lời Đọc: `reading_score = (correct/40) × 10` (làm tròn tới 0.5)
...
- **Mô tả**: Khóa bài nộp để giảng viên hiện tại đánh giá độc quyền.
- **Đầu vào**: ID bài nộp
- **Xử lý**: Lấy khóa phân tán Redis `lock:review:{submissionId}` (TTL 15 phút) → thiết lập `claimed_by` và `claimed_at`
...
- **Đầu vào**: ID bài nộp
- **Xử lý**: Xóa khóa Redis → xóa `claimed_by` và `claimed_at`
...
- **Xử lý**:
  ...
  6. Giải phóng khóa Redis
  7. Phát sóng SSE sự kiện `completed`
  8. Kích hoạt tính lại tiến độ cho học viên
- **Đầu ra**: Bài nộp đã cập nhật với điểm cuối cùng
...
- **Cập nhật**: Chỉnh sửa nội dung câu hỏi. Tạo snapshot phiên bản mới trong `question_versions`.
- **Xóa**: Quản trị viên xóa mềm bằng cách thiết lập `is_active = false`.

#### 3.13.2 Nhập/Xuất

- **Mô tả**: Nhập hàng loạt câu hỏi từ định dạng JSON/Excel, xuất để sao lưu.
- **Nhập**: Xác thực theo Zod schema cho từng kỹ năng → chèn với version 1
- **Xuất**: Tạo JSON/Excel với toàn bộ dữ liệu câu hỏi
...
- **Tạo Hàng Loạt**: Tạo nhiều tài khoản người dùng từ CSV/Excel
- **Khóa/Mở Khóa**: Vô hiệu hóa/kích hoạt quyền truy cập tài khoản người dùng
- **Đặt Lại Mật Khẩu**: Đặt lại mật khẩu do quản trị viên khởi tạo

### 3.15 FE-14: Bảng Phân Tích

#### 3.15.1 Bảng Điều Khiển Giảng Viên

- **Mô tả**: Xem thống kê lớp học và tiến độ học viên.
- **Đầu ra**: Số học viên hoạt động, tỷ lệ hoàn thành bài tập, điểm trung bình theo kỹ năng, tóm tắt tiến độ mỗi học viên

#### 3.15.2 Bảng Điều Khiển Quản Trị Viên

- **Mô tả**: Phân tích toàn hệ thống.
- **Đầu ra**: Tổng người dùng theo vai trò, phân bố bài nộp theo trạng thái, thời gian chấm điểm trung bình, kích thước hàng đợi đánh giá, người dùng hoạt động hàng ngày

### 3.16 FE-15: Hệ Thống Thông Báo

#### 3.16.1 Loại Thông Báo
...
### 3.17 FE-16: Tính Năng Quản Trị Nâng Cao

#### 3.17.1 Lịch Sử Hoạt Động

- **Mô tả**: Xem nhật ký hoạt động người dùng (đăng nhập, bài nộp, đánh giá).

#### 3.17.2 Phân Phối Bài Chấm Tự Động

- **Mô tả**: Tự động phân phối bài nộp review_pending cho các giảng viên khả dụng dựa trên khối lượng công việc.
...
|----------------|-----------|---------|
| Groq API (Llama 3.3 70B) | HTTPS REST | Chấm điểm Viết/Nói dựa trên LLM |
| Groq API (Whisper Large V3 Turbo) | HTTPS REST | Phiên âm Speech-to-Text |
| PostgreSQL 17 | TCP 5432 | Kho dữ liệu chính |
| Redis 7.2+ | TCP 6379 | Hàng đợi, bộ nhớ đệm, khóa phân tán |
...
- **Thời gian thực**: SSE cho cập nhật trạng thái chấm điểm (một chiều server → client)
...
| REQ-R02 | Worker chấm điểm thử lại: tối đa 3 lần với exponential backoff (arq xử lý nội bộ) |
...
| REQ-R07 | SSE kết nối lại với `Last-Event-ID` phát lại — không bỏ lỡ sự kiện chấm điểm |
...
| REQ-P07 | SSE heartbeat: mỗi 30 giây, thời gian kết nối tối đa 30 phút |
...
| REQ-S06 | Quyền truy cập dữ liệu bài nộp giới hạn cho chủ sở hữu (Row Level Security + Secure Views) |
...
| REQ-S09 | Hỗ trợ Idempotency-Key trên `POST /submissions` để ngăn bài nộp trùng lặp khi thử lại |

#### 4.2.5 Khả Năng Mở Rộng

| Yêu cầu | Đặc tả |
|-------------|---------------|
| REQ-SC01 | Worker chấm điểm có thể mở rộng ngang — nhiều arq worker tiêu thụ từ cùng hàng đợi Redis |
| REQ-SC02 | Index cơ sở dữ liệu tối ưu cho các mẫu truy vấn phổ biến (lịch sử người dùng, hàng đợi đánh giá, câu hỏi hoạt động) |
| REQ-SC03 | SSE hub sử dụng pub/sub trong bộ nhớ với buffer có giới hạn (100 sự kiện) và backpressure |
...
|----|-----------------|
| BR-01 | Điểm Nghe = `(correct / 35) × 10`, làm tròn tới 0.5 gần nhất |
| BR-02 | Điểm Đọc = `(correct / 40) × 10`, làm tròn tới 0.5 gần nhất |
...
| BR-19 | Khóa claim đánh giá: Redis TTL 15 phút. Tự động giải phóng sau khi hết hạn |
...
| CR-08 | Hỗ trợ Idempotency qua header `Idempotency-Key` trên endpoint POST có tác dụng phụ |

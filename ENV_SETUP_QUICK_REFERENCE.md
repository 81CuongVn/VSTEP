# Quick Environment Setup Reference

## Copy-Paste for Render Backend

Go to: **Render Dashboard → Your Service → Environment**

Add these variables (copy the entire block):

```
APP_NAME=Laravel
APP_ENV=production
APP_KEY=base64:B/ScOYdZCJTB13gmYqUxqVhEXdft3lSX6BvbCKiMX/0=
APP_DEBUG=false
APP_URL=https://vstep.onrender.com
APP_LOCALE=en
APP_FALLBACK_LOCALE=en
APP_FAKER_LOCALE=en_US
BCRYPT_ROUNDS=12
LOG_CHANNEL=stack
LOG_STACK=single
LOG_DEPRECATIONS_CHANNEL=null
LOG_LEVEL=debug
DB_CONNECTION=pgsql
DB_SEED_ON_BOOT=true
DB_SSLMODE=require
DB_URL=postgresql://neondb_owner:npg_1cgxi3oODUed@ep-gentle-field-a1ldpcht-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require
DB_URL_DIRECT=postgresql://neondb_owner:npg_1cgxi3oODUed@ep-gentle-field-a1ldpcht.ap-southeast-1.aws.neon.tech/neondb?sslmode=require
SESSION_DRIVER=database
SESSION_LIFETIME=120
SESSION_ENCRYPT=false
SESSION_PATH=/
SESSION_DOMAIN=null
BROADCAST_CONNECTION=log
FILESYSTEM_DISK=local
QUEUE_CONNECTION=sync
CACHE_STORE=database
REDIS_URL=redis://default:gQAAAAAAAUzlAAIncDJlMDlmMzUxYzE1MjQ0NWExYmIxODI3NmZlNTJhOThlYXAyODUyMjE@main-newt-85221.upstash.io:6379
MAIL_MAILER=log
MAIL_SCHEME=null
MAIL_HOST=127.0.0.1
MAIL_PORT=2525
MAIL_USERNAME=null
MAIL_PASSWORD=null
MAIL_FROM_ADDRESS=hello@example.com
MAIL_FROM_NAME=Laravel
AWS_ACCESS_KEY_ID=
AWS_SECRET_ACCESS_KEY=
AWS_DEFAULT_REGION=auto
AWS_BUCKET=vstep
AWS_ENDPOINT=https://<account_id>.r2.cloudflarestorage.com
AWS_USE_PATH_STYLE_ENDPOINT=true
AZURE_SPEECH_KEY=
AZURE_SPEECH_REGION=southeastasia
OPENAI_API_KEY=
OPENAI_BASE_URL=
CORS_ALLOWED_ORIGINS=https://vstep.hamhochoi.com,https://vstep.vercel.app
CORS_ALLOWED_ORIGIN_PATTERNS=^https://.*\.vercel\.app$
FRONTEND_URL=https://vstep.hamhochoi.com
JWT_SECRET=01wW0Hc2srXnDjjl2oA70Z7GwtVZZMza7vCNuV8H7c95SLufVWt6DRu98MArkaNUQUEUE
OCTANE_SERVER=frankenphp
OCTANE_HTTPS=false
OCTANE_HOST=0.0.0.0
OCTANE_PORT=8000
OCTANE_ADMIN_HOST=127.0.0.1
OCTANE_ADMIN_PORT=2019
OCTANE_WORKERS=auto
OCTANE_MAX_REQUESTS=500
```

---

## Copy-Paste for Vercel Frontend

Go to: **Vercel Dashboard → Your Project → Settings → Environment Variables**

Add these variables:

| Variable | Value |
|----------|-------|
| `VITE_API_URL` | `https://vstep.onrender.com` |
| `VITE_STORAGE_URL` | `https://pub-44427da338f348eca0451808ade7798e.r2.dev` |

---

## Variables That Need Your Own Values

Replace these with your actual credentials:

### In Render:
- `AWS_ACCESS_KEY_ID` → Your R2 access key
- `AWS_SECRET_ACCESS_KEY` → Your R2 secret key
- `AWS_ENDPOINT` → Replace `<account_id>` with your Cloudflare account ID
- `AZURE_SPEECH_KEY` → Your Azure Speech API key (if using)
- `OPENAI_API_KEY` → Your OpenAI API key (if using)

---

## Deployment Steps

### 1. Update Render Backend
```bash
# Push your code with the fixed files:
# - apps/backend-v2/app/Services/SessionService.php
# - apps/backend-v2/app/Http/Resources/ExamSessionDetailResource.php
# - apps/backend-v2/.env

git add apps/backend-v2/
git commit -m "fix: exam start endpoint - handle hidden blueprint and null relationships"
git push
```

Render will automatically redeploy.

### 2. Update Vercel Frontend
```bash
# Push your code with the fixed files:
# - apps/frontend/.env

git add apps/frontend/.env
git commit -m "fix: update backend API URL to Render"
git push
```

Vercel will automatically redeploy.

### 3. Verify Deployment
- Check Render logs: No database errors
- Check Vercel logs: No build errors
- Test in browser: Exam start should work (200 response)

---

## Environment Variable Descriptions

| Variable | Purpose | Example |
|----------|---------|---------|
| `APP_ENV` | Laravel environment | `production` |
| `APP_DEBUG` | Show detailed errors | `false` (production) |
| `APP_URL` | Backend URL for redirects | `https://vstep.onrender.com` |
| `DB_URL` | PostgreSQL connection string | `postgresql://user:pass@host/db` |
| `CORS_ALLOWED_ORIGINS` | Allowed frontend domains | `https://vstep.vercel.app` |
| `JWT_SECRET` | Token signing key | 64-char random string |
| `VITE_API_URL` | Backend URL for frontend | `https://vstep.onrender.com` |
| `VITE_STORAGE_URL` | R2 storage URL for media | `https://pub-xxx.r2.dev` |

---

## Testing After Deployment

1. **Backend Health Check**
   ```
   curl https://vstep.onrender.com/health
   ```

2. **Frontend Loads**
   - Visit https://vstep.hamhochoi.com
   - Check browser console for errors

3. **API Communication**
   - Open DevTools → Network tab
   - Try to start an exam
   - Should see POST to `/api/v1/exams/.../start` with 200 response

4. **Database Connection**
   - Check Render logs for "Connected to database"
   - No "Connection refused" errors

---

## Rollback if Issues Occur

If something breaks after deployment:

1. **Render**: Go to Deployments, click "Rollback" on previous version
2. **Vercel**: Go to Deployments, click "Promote to Production" on previous version

Then investigate the issue before redeploying.

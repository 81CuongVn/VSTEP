...
export default function ClassesScreen() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setFeedback({ type: "error", message: err?.message ?? "Không thể tham gia lớp" });
    }
  };
  ...
  const renderClass = ({ item }: { item: Class }) => (
    ...
    >
      ...
      <View style={styles.cardInfo}>
        <Text style={[styles.cardName, { color: c.foreground }]}>{item.name}</Text>
        {item.description ? (
      ...
      <BouncyFlatList
        ...
        ListHeaderComponent={
          <View>
            ...
            {/* Join class section */}
            <View style={[styles.joinSection, { backgroundColor: c.card, borderColor: c.border }]}>
              <Text style={[styles.joinTitle, { color: c.foreground }]}>Tham gia lớp</Text>
              ...
              {feedback && (
                <Text
                  style={[
                    styles.feedback,
                    { color: feedback.type === "success" ? c.success : c.destructive },
                  ]}
                >
                  {feedback.message}
                </Text>
            ...
            <Text style={[styles.sectionTitle, { color: c.foreground }]}>Danh sách lớp</Text>
          </View>
        }
        ListEmptyComponent={
          <EmptyState
...
const styles = StyleSheet.create({
  ...
  joinTitle: { fontSize: fontSize.sm, fontWeight: "600", marginBottom: spacing.sm },
